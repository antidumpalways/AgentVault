...
const VERBS = ["REMEMBER", "REASON", "SECURE", "MONETIZE", "EVOLVE"];
        ...
        <div className="grid lg:grid-cols-[1fr] gap-12 lg:gap-20 items-start">
          <div>
            ...
            <div className={`mt-14 transition-all duration-700 delay-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              <p className="text-base text-[#5a5a5a] leading-relaxed max-w-xl">Store, secure, and monetize AI agent memory. AgentVault is the encrypted memory layer that lets agents learn from the past, make better decisions, and share knowledge—all with cryptographic proof of ownership.</p>
              <div className="flex flex-col sm:flex-row gap-3 mt-8 w-fit">
                ...
                <a href="#features" className="group inline-flex items-center gap-8 border border-[#1e1e1e] text-[#f2ede6] font-mono text-sm tracking-widest px-6 py-4 hover:border-[#00d9ff]/40 hover:text-[#00d9ff] transition-colors whitespace-nowrap">EXPLORE DOCS<span className="transition-transform group-hover:translate-x-1">→</span></a>
              </div>
              <div className="flex items-center gap-3 mt-5">
                ...
                <span className="font-mono text-[10px] text-[#3a3a3a]">1,200+ agents secured this month</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="marquee-fast whitespace-nowrap flex gap-16">
            {[...Array(2)].map((_, rep) => (
              <span key={rep} className="inline-flex items-center gap-16">
                {["END-TO-END ENCRYPTION","PERSISTENT MEMORY LAYER","AGENT KNOWLEDGE SHARING","CRYPTOGRAPHIC PROOFS","ZERO-KNOWLEDGE ARCHITECTURE","DECENTRALIZED OWNERSHIP","MONETIZATION READY"].map(item => (
